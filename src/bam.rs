...
pub fn query_bam_records_for_region(
    ...
    if debug {
        println!(
            "    DEBUG: BAM query - file: {bam_path:?}, region: {chrom}:{start_pos}-{end_pos}"
        );
    }
    ...
    let mut indexed_reader =
        bam::io::indexed_reader::Builder::default().build_from_path(bam_path)?;
    ...
    for result in query {
        ...
        let (read_start, read_end) = if let Some(Ok(alignment_start)) = record.alignment_start() {
            ...
            let read_end = if reference_pos > start {
                reference_pos - 1
            } else {
                start
            };
        ...
        if debug && record_count <= 5 {
            ...
            println!(
                "    DEBUG: Stored record #{record_count}: {read_id} at {chrom}:{read_start}-{read_end} (mapq={mapping_quality}, timestamp={timestamp})"
            );
    ...
    if debug {
        println!(
            "    DEBUG: BAM query completed - found {} total records for region {}:{}-{}",
            records.len(),
            chrom,
            start_pos,
            end_pos
        ...
        if records.len() > 5 {
            println!(
                "    DEBUG: Additional {} reads found (showing first 5 above)",
                records.len() - 5
            );
            println!(
                "    DEBUG: Read position range: {}:{}-{} to {}:{}-{}",
                chrom,
                records
                    .iter()
